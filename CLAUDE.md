...
- `CreateSessionRequest` — Session creation payload (`gameKey` serialized as `gameTypeConfigKey`, `customVariables` serialized as `environmentVariables` — both via `[JsonProperty]`; plus region, channel)
